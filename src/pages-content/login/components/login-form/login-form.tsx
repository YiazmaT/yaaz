...
export function LoginForm(props: LoginFormProps) {
  ...
  async function submit(data: typeof defaultValues) {
    setLoading(true);
    try {
      await props.onSubmit(data.login, data.password);
    } finally {
      setLoading(false);
    }
  }
                  ...
                  {loading && (
                    <>
                      <svg width={0} height={0}>
                        <defs>
                          <linearGradient id={props.gradientId} x1="0%" y1="0%" x2="0%" y2="100%">
                      ...
                      <CircularProgress
                        disableShrink
                        sx={{position: "absolute", "svg circle": {stroke: `url(#${props.gradientId})`}}}
                        size={300}
                        thickness={1}
                      />
                    </>
