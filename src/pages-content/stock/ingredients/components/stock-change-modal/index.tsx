...
export function StockChangeModal(props: StockChangeModalProps) {
  ...
  async function handleSubmit() {
    ...
    if (reason === IngredientStockChangeReason.other && !comment.trim()) {
      toast.errorToast("ingredients.stockChange.commentRequired");
      return;
    }
    ...
    await api.fetch("POST", "/api/stock/ingredient/stock-change", {
      ...
      onSuccess: () => {
        toast.successToast("ingredients.stockChange.success");
        handleClose();
        onSuccess();
      },
    });
  ...
  return (
    <GenericDrawer title="ingredients.stockChange.title" show={!!item} onClose={handleClose}>
      <Grid container spacing={2}>
        ...
        <Grid size={12}>
          <Typography variant="body1" color="text.secondary">
            {translate("ingredients.stockChange.currentStock")}:{" "}
            <strong>
              {currentStock.toLocaleString("pt-BR")} {unitLabel}
            </strong>
          </Typography>
        </Grid>

        <Grid size={12}>
          <DecimalInput label="ingredients.stockChange.newStock" value={newStock} onChange={setNewStock} fullWidth />
        </Grid>
        ...
        {!hasNoChange && (
          <Grid size={12}>
            <Typography variant="body2" sx={{color: isAdding ? "success.main" : "error.main", fontWeight: 500}}>
              {isAdding
                ? `${translate("ingredients.stockChange.adding")}: ${Math.abs(stockDifference).toLocaleString("pt-BR")} ${unitLabel}`
                : `${translate("ingredients.stockChange.removing")}: ${Math.abs(stockDifference).toLocaleString("pt-BR")} ${unitLabel}`}
            </Typography>
          </Grid>
        )}

        <Grid size={12}>
          <RadioGroup label="ingredients.stockChange.reason" value={reason} onChange={setReason} options={reasonOptions} />
        </Grid>

        <Grid size={12}>
          <TextField
            label={translate("ingredients.stockChange.comment")}
            value={comment}
            onChange={(e) => setComment(e.target.value)}
            fullWidth
            multiline
            rows={3}
            required={reason === IngredientStockChangeReason.other}
          />
        </Grid>
