...
export function CostHistoryModal(props: CostHistoryModalProps) {
  ...
  const {data: result, isLoading} = useApiQuery<CostHistoryItem[]>({
    ...
    route: `/api/stock/package/${props.packageId}/cost-history`,
  ...
  return (
    <GenericModal open={props.open} onClose={props.onClose} title={`${translate("packages.costHistory")} - ${props.packageName}`} maxWidth="md">
      {isLoading ? (
