...
export function CostHistoryModal(props: CostHistoryModalProps) {
  ...
  const {data: result, isLoading} = useApiQuery<CostHistoryItem[]>({
    queryKey: ["cost-history", "product", props.productId],
    route: `/api/stock/product/${props.productId}/cost-history`,
  ...
  return (
    <GenericModal
      ...
      title={`${translate("products.costHistory")} - ${props.productName}`}
      maxWidth="md"
    >
      {isLoading ? (
